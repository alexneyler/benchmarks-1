...
export async function runConcurrentBenchmark(config: ConcurrentConfig): Promise<ConcurrentBenchmarkResult> {
  const { name, concurrency, timeout = 120_000, requiredEnvVars, sandboxOptions, destroyTimeoutMs, task, taskTimeoutMs } = config;
  ...
  const promises = Array.from({ length: concurrency }, (_, i) =>
    runIteration(compute, timeout, sandboxOptions, destroyTimeoutMs, reuseDetector, task, taskTimeoutMs)
      .then(result => {
