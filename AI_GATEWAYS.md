...
- **Gemini family** (`ai-gateway-gemini.bench.ts` + `providers-gemini.ts`) — seven gateways (OpenRouter, Vercel AI Gateway, Cloudflare AI Gateway, LLM Gateway, Concentrate AI, Neon AI Gateway, and ngrok AI Gateway; Pydantic, Novita, and Ramp Router excluded — see below) routed to `gemini-3.6-flash`, measured against a direct-to-Gemini baseline. See [Gemini family benchmark](#gemini-family-benchmark).
...
- **Model**: Claude Haiku 4.5 for the ten Anthropic-family gateway-overhead participants — `anthropic/claude-haiku-4.5` via OpenRouter's and Vercel AI Gateway's catalog alias, `anthropic/claude-haiku-4-5` via LLM Gateway's provider-pinned catalog naming, `anthropic/claude-haiku-4-5-20251001` via Concentrate AI's provider-prefixed naming, `claude-haiku-4-5-20251001` via Cloudflare's, Anthropic's own, and Pydantic AI Gateway's native model ID, `claude-haiku-4-5` via Neon AI Gateway (Pydantic proxies Anthropic's native API as-is, no gateway-specific model prefix), and `claude-haiku-4-5` via ngrok AI Gateway (`/v1/messages`). Novita and Ramp Router are included with the same family model; both are expected to fail at runtime because neither catalog carries Anthropic models. Same underlying model, addressed the way each API expects it to be addressed. The OpenAI family uses `gpt-5.4-mini` instead — see [OpenAI family benchmark](#openai-family-benchmark); the Gemini family uses `gemini-3.6-flash` — see [Gemini family benchmark](#gemini-family-benchmark); the Kimi family uses `kimi-k3` — see [Kimi family benchmark](#kimi-family-benchmark).
