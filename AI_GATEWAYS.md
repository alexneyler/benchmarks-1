...
- **Model**: Claude Haiku 4.5 for the ten Anthropic-family gateway-overhead participants — `anthropic/claude-haiku-4.5` via OpenRouter's and Vercel AI Gateway's catalog alias, `anthropic/claude-haiku-4-5` via LLM Gateway's provider-pinned catalog naming, `anthropic/claude-haiku-4-5-20251001` via Concentrate AI's provider-prefixed naming, `claude-haiku-4-5-20251001` via Cloudflare's, Anthropic's own, Pydantic AI Gateway's, and LLM API's native model ID (`/v1/messages`), `claude-haiku-4-5` via Neon AI Gateway (Pydantic proxies Anthropic's native API as-is, no gateway-specific model prefix), and `claude-haiku-4-5` via ngrok AI Gateway (`/v1/messages`). Ramp Router uses the account-specific catalog id `claude-haiku-4-5` via `/v1/responses`. Same underlying model, addressed the way each API expects it to be addressed. The OpenAI family uses `gpt-5.4-mini` instead — see [OpenAI family benchmark](#openai-family-benchmark); the Gemini family uses `gemini-3.6-flash` — see [Gemini family benchmark](#gemini-family-benchmark); the Kimi family uses `kimi-k3` — see [Kimi family benchmark](#kimi-family-benchmark).
...
| LLM API | `gpt-5.4-mini` via `/v1/responses` | Medium — the `/v1/responses` endpoint is present on `api.llmapi.ai`, but LLM API's public docs focus on the OpenAI-compatible chat-completions surface and do not specifically document the Responses API; `store: false` is sent by `phase-probe.ts` like every other Responses request | Unconfirmed — no public documentation specifically covers the Responses API streaming path on LLM API |
...
| LLM API | `gemini-3.6-flash` via `/v1/chat/completions` | Medium — `gemini-3.6-flash` is the public model id in LLM API's `/v1/models` catalog, but the exact request-body pin string and streaming behavior on this OpenAI-compatible route have not been confirmed live |
...
| LLM API | `kimi-k3` via `/v1/chat/completions` | Medium — `kimi-k3` is the public model id in LLM API's `/v1/models` catalog; uses the same `temperature: undefined` and `reasoningCountsAsFirstToken: true` overrides as the other Kimi-family `openai`-format entries |
