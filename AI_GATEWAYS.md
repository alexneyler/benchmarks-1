...
- **OpenAI family** (`ai-gateway-openai.bench.ts` + `providers-openai.ts`) — every gateway routed to `gpt-5.4-mini` instead, measured against a direct-to-OpenAI baseline. See [OpenAI family benchmark](#openai-family-benchmark).
...
# OpenAI family — nine gateways + openai-direct, routed to gpt-5.4-mini instead
