...
- **OpenAI family** (`ai-gateway-openai.bench.ts` + `providers-openai.ts`) — every gateway except Pydantic AI Gateway, routed to `gpt-5.4-mini` instead, measured against a direct-to-OpenAI baseline. See [OpenAI family benchmark](#openai-family-benchmark).
- **Gemini family** (`ai-gateway-gemini.bench.ts` + `providers-gemini.ts`) — eight gateways (OpenRouter, Vercel AI Gateway, Cloudflare AI Gateway, LLM Gateway, Concentrate AI, Novita, Ramp Router, and Neon AI Gateway; Pydantic excluded — see below) routed to `gemini-3.6-flash`, measured against a direct-to-Gemini baseline. See [Gemini family benchmark](#gemini-family-benchmark).
- **Kimi family** (`ai-gateway-kimi.bench.ts` + `providers-kimi.ts`) — eight gateways (OpenRouter, Vercel AI Gateway, Cloudflare AI Gateway, LLM Gateway, Concentrate AI, Novita, Ramp Router, and Neon AI Gateway; Pydantic excluded — see below) routed to `kimi-k3`, measured against a direct-to-Moonshot baseline. See [Kimi family benchmark](#kimi-family-benchmark).
...
- **Model**: Claude Haiku 4.5 for the nine Anthropic-family gateway-overhead participants — `anthropic/claude-haiku-4.5` via OpenRouter's and Vercel AI Gateway's catalog alias, `anthropic/claude-haiku-4-5` via LLM Gateway's provider-pinned catalog naming, `anthropic/claude-haiku-4-5-20251001` via Concentrate AI's provider-prefixed naming, `claude-haiku-4-5-20251001` via Cloudflare's, Anthropic's own, and Pydantic AI Gateway's native model ID, and `claude-haiku-4-5` via Neon AI Gateway (Pydantic proxies Anthropic's native API as-is, no gateway-specific model prefix). Novita and Ramp Router are included with the same family model; both are expected to fail at runtime because neither catalog carries Anthropic models. Same underlying model, addressed the way each API expects it to be addressed. The OpenAI family uses `gpt-5.4-mini` instead — see [OpenAI family benchmark](#openai-family-benchmark); the Gemini family uses `gemini-3.6-flash` — see [Gemini family benchmark](#gemini-family-benchmark); the Kimi family uses `kimi-k3` — see [Kimi family benchmark](#kimi-family-benchmark).
...
- **`openai`** (OpenRouter, Vercel AI Gateway, Cloudflare AI Gateway, LLM Gateway, Concentrate AI, Novita, and Neon in the Kimi family; OpenRouter, Vercel AI Gateway, LLM Gateway, Concentrate AI, and Novita in the Gemini family; OpenRouter, Vercel AI Gateway, LLM Gateway, and Novita in the Anthropic family) — OpenAI-compatible `/chat/completions` shape, `Authorization: Bearer <key>`. Kimi's own API is itself natively OpenAI-Chat-Completions-shaped (Moonshot's own design, not a third-party compatibility shim), so this format is a no-translation-layer native route for the Kimi participants that use it. For the Gemini family's five participants using this format, it genuinely is a translation layer — see [Gemini family benchmark](#gemini-family-benchmark) for why no native alternative was available. Ramp Router uses `responses` for Kimi because it has no `/chat/completions` route.
- **`anthropic`** (Cloudflare AI Gateway, Anthropic direct, Pydantic AI Gateway, Vercel AI Gateway, LLM Gateway, and Neon AI Gateway) — Anthropic's native `/v1/messages` shape. Auth header varies within this group: Cloudflare and Anthropic direct use `x-api-key` + `anthropic-version`; Pydantic AI Gateway, Vercel AI Gateway, LLM Gateway, and Neon AI Gateway use `Authorization: Bearer <key>` + `anthropic-version` instead — for Pydantic this was confirmed directly against a real request (its own auth failures return a same-shaped 401 regardless of which of the two header styles is wrong, so this took a few rounds of live testing to pin down precisely). Concentrate AI's `/v1/messages/` endpoint is documented as an "Anthropic Messages API compatibility endpoint" in its published OpenAPI spec (`concentrate.ai/docs/api-reference/openapi.json`), but has **not** been confirmed against a real successful response — see the note in `providers.ts` and in Limitations below.
- **`responses`** (Concentrate AI and OpenAI direct in the Anthropic family; every participant in the OpenAI family except Novita; Ramp Router in the Kimi family and Gemini family — see [OpenAI family benchmark](#openai-family-benchmark)) — OpenAI's Responses API shape: flat `input` string instead of a `messages` array, `max_output_tokens` instead of `max_tokens`. For OpenAI direct this is the format's origin, called directly — OpenAI's current flagship endpoint (rather than the older Chat Completions surface). Most OpenAI-family participants use it because they have a Responses passthrough; Novita is included via `/chat/completions` so the unsupported-model failure is still captured.
...
**Every participant in this family except Novita uses the OpenAI Responses API**; Novita is hit via its OpenAI-compatible `/chat/completions` route because it has no Responses API, and is included so an unsupported-model failure is captured rather than silently excluded. The policy remains: use a gateway's Responses passthrough if it has one (Responses is OpenAI's own format, so proxying it natively adds no translation layer — the same reasoning that picked `/v1/messages` over the OpenAI-compatible route for the Anthropic family's Cloudflare/Pydantic/LLM Gateway/Vercel entries), fall back to Chat Completions only where no Responses route is documented.
...
| Novita | `gpt-5.4-mini` via `/openai/v1/chat/completions` | Low — Novita's catalog does not list OpenAI models; this entry is included so the unsupported-model failure is captured rather than silently excluded | N/A — expected to fail at runtime |
...
**Two gateways get the native `wireFormat: 'gemini'` treatment: Cloudflare AI Gateway and Neon AI Gateway.** Unlike the OpenAI family, where most gateways turned out to have a native Responses passthrough, OpenRouter, Vercel AI Gateway, LLM Gateway, Concentrate AI, and Novita show no documented native Gemini passthrough (in contrast to their confirmed native Anthropic Messages / OpenAI Responses routes used elsewhere in this repo) — each is built around one normalized endpoint across its whole catalog instead. Those five route through that OpenAI-compatible `/chat/completions` surface with the family target model, translating where the gateway can or failing at runtime if it cannot. Ramp Router is included via `/v1/responses` and is expected to fail at runtime because it has no Gemini provider.
...
| Novita | `gemini-3.6-flash` via `/openai/v1/chat/completions` | Low — Novita's catalog does not list Gemini models; this entry is included so the unsupported-model failure is captured rather than silently excluded |
| Ramp Router | `gemini-3.6-flash` via `/v1/responses` | Low — Ramp Router has no Gemini provider; included so the unsupported-model failure is captured rather than silently excluded |
...
```
round 1: openrouter → vercel-ai-gateway → cloudflare-ai-gateway → llmgateway → pydantic-ai-gateway → concentrate-ai-gateway → novita → ramp → neon → anthropic-direct
round 2: openrouter → vercel-ai-gateway → cloudflare-ai-gateway → llmgateway → pydantic-ai-gateway → concentrate-ai-gateway → novita → ramp → neon → anthropic-direct
...
```
...
```bash
# Anthropic family — all ten participants (nine gateway-overhead + anthropic-direct),
...
# OpenAI family — eight gateways + openai-direct, routed to gpt-5.4-mini instead
...
pnpm run bench:ai-gateway-openai:novita
pnpm run bench:ai-gateway-openai:ramp
pnpm run bench:ai-gateway-openai:neon
pnpm run bench:ai-gateway-openai:direct

# Gemini family — eight gateways (no Pydantic — see Gemini family benchmark above)
...
pnpm run bench:ai-gateway-gemini:novita
pnpm run bench:ai-gateway-gemini:ramp
pnpm run bench:ai-gateway-gemini:neon
pnpm run bench:ai-gateway-gemini:direct

# Kimi family — eight gateways (no Pydantic — see Kimi family benchmark above)
