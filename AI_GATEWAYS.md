...
- **OpenAI family** (`ai-gateway-openai.bench.ts` + `providers-openai.ts`) — eight gateways (OpenRouter, Vercel AI Gateway, Cloudflare AI Gateway, LLM Gateway, Concentrate AI, Pydantic AI Gateway, Ramp Router, and Neon AI Gateway; Novita excluded — see below) routed to `gpt-5.4-mini` instead, measured against a direct-to-OpenAI baseline. See [OpenAI family benchmark](#openai-family-benchmark).
- **Gemini family** (`ai-gateway-gemini.bench.ts` + `providers-gemini.ts`) — six gateways (OpenRouter, Vercel AI Gateway, Cloudflare AI Gateway, LLM Gateway, Concentrate AI, and Neon AI Gateway; Pydantic, Novita, and Ramp Router excluded — see below) routed to `gemini-3.6-flash`, measured against a direct-to-Gemini baseline. See [Gemini family benchmark](#gemini-family-benchmark).
...
- **`openai`** (OpenRouter, Vercel AI Gateway, Cloudflare AI Gateway, LLM Gateway, Concentrate AI, Novita, and Neon in the Kimi family; OpenRouter, Vercel AI Gateway, LLM Gateway, and Concentrate AI in the Gemini family; OpenRouter and Novita in the Anthropic family) — OpenAI-compatible `/chat/completions` shape, `Authorization: Bearer <key>`. Kimi's own API is itself natively OpenAI-Chat-Completions-shaped (Moonshot's own design, not a third-party compatibility shim), so this format is a no-translation-layer native route for the Kimi participants that use it. For the Gemini family's four participants using this format, it genuinely is a translation layer — see [Gemini family benchmark](#gemini-family-benchmark) for why no native alternative was available. Ramp Router uses `responses` for Kimi because it has no `/chat/completions` route.
...
**Two gateways get the native `wireFormat: 'gemini'` treatment: Cloudflare AI Gateway and Neon AI Gateway.** Unlike the OpenAI family, where most gateways turned out to have a native Responses passthrough, OpenRouter, Vercel AI Gateway, LLM Gateway, and Concentrate AI show no documented native Gemini passthrough (in contrast to their confirmed native Anthropic Messages / OpenAI Responses routes used elsewhere in this repo) — each is built around one normalized endpoint across its whole catalog instead. Those four route through that OpenAI-compatible `/chat/completions` surface with the family target model, translating where the gateway can or failing at runtime if it cannot.
...
# Gemini family — six gateways (Pydantic, Novita, and Ramp Router excluded — see Gemini family benchmark above)
