...
import { sortAIGatewayByCompositeScore, computeAIGatewayCompositeScores } from './scoring.js';
...
function formatProviderName(s: string): string {
  ...
  if (s === 'netlify') return 'Netlify AI Gateway';
...
function generateSVG(
  ...
  const sorted = sortAIGatewayByCompositeScore(results).filter(r => !r.skipped);
...
`;
  ...
  sorted.forEach((r, i) => {
    ...
    const rank = i + 1;
    const coldE2eMs = r.summary.coldE2eMs.median;
    const allFailed = ok === 0;
    const score = r.compositeScore !== undefined ? r.compositeScore.toFixed(1) : '--';

    let speedClass = allFailed ? 'slow' : 'fast';
    if (!allFailed && coldE2eMs > 2000) speedClass = 'slow';
    else if (!allFailed && coldE2eMs > 1000) speedClass = 'medium';
