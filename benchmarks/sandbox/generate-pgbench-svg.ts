/**
 * Render pgbench SVG bar chart from results/pgbench/latest.json.
...
import { SUITE_CONFIG, type PgbenchBenchmarkResult } from './pgbench.js';
...
function readLatest(): PgbenchBenchmarkResult[] | null {
  const dir = path.join(RESULTS_DIR, 'pgbench');
...
function main(): void {
  ...
  const name = 'pgbench.svg';
  const target = path.join(OUT_DIR, name);
  fs.writeFileSync(target, svg);
  console.log('Wrote ' + name + ' (' + svg.length + ' bytes)');
}

if (process.argv[1] && process.argv[1].endsWith('generate-pgbench-svg.ts')) {
  main();
}
