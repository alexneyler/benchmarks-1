...
describe('scoring: realworld', () => {
  ...
  test('computeStats handles all-failure iterations', () => {
    const results: WorkloadResult[] = [
      { ok: false, suite: 'realworld', reason: 'error', error: 'test', meta: {} },
      { ok: false, suite: 'realworld', reason: 'timeout', error: 'test', meta: {} },
    ];
  ...
  test('computeStats produces median and score for successful runs', () => {
    ...
    const results: WorkloadResult[] = [
      { ok: true, suite: 'realworld', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
      { ok: true, suite: 'realworld', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
      { ok: true, suite: 'realworld', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
    ];
