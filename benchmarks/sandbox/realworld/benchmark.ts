...
import { getBundlePath, getFixtureVersion } from './util/upload-bundle.js';
import { ensureBundleForRun } from '../../scripts/ensure-bundle.js';

/**
 * Run one (provider) cell at the given replicate count for realworld.
 *
 * Each replicate:
...
function buildPayload(): Payload | PayloadErr {
  ...
  if (SUITE_CONFIG.bundle !== 'none') {
    const bundlePath = getBundlePath(SUITE_CONFIG.bundle as 'fixture-archive');
    if (!fs.existsSync(bundlePath)) {
      return { ok: false, error: 'Bundle missing on disk: ' + bundlePath + '. Run \`pnpm run build:bundles\`.' };
    }
    bundleB64 = fs.readFileSync(bundlePath).toString('base64');
    bundleLarge = bundleB64.length > BUNDLE_INLINE_LIMIT;
  }
  const fixtureVersion = SUITE_CONFIG.bundle === 'fixture-archive' ? getFixtureVersion() : undefined;

  return {
    ok: true,
    scriptName: path.basename(SUITE_CONFIG.workloadPath),
    scriptContent, stdoutContent,
    bundleB64, bundleLarge, fixtureVersion,
  };
}
