...
const SUITE_CONFIG = {
  id: 'network-wan' as const,
  label: 'Network WAN throughput',
  unit: 'mb_per_s' as const,
  higherIsBetter: true,
  ceiling: 170,
  defaultReplicas: 3,
  workloadPath: 'network-wan-workload.js',
  timeoutMs: 60_000,
};

const BENCH_SCRIPT_PATH = path.resolve(import.meta.dirname, '../scripts/network-wan-workload.js');
const BENCH_STDOUT_PATH = path.resolve(import.meta.dirname, '../scripts/network-wan-stdout.js');

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

export interface NetworkWanWorkloadMeta {
  cpuCount?: number;
  memoryMb?: number;
  workloadMs?: number;
...
export type NetworkWanWorkloadResult =
  | {
      ok: true;
      suite: 'network-wan';
      metric: { value: number; unit: 'mb_per_s'; higherIsBetter: boolean };
      meta: NetworkWanWorkloadMeta;
      notes?: string;
    }
  | {
      ok: false;
...
export interface NetworkWanSuiteStats {
  median: number;
  p95: number;
  p99: number;
  min: number;
...
export interface NetworkWanBenchmarkResult {
  provider: string;
  suite: 'network-wan';
  mode: 'network-wan';
  iterations: NetworkWanWorkloadResult[];
  summary: NetworkWanSuiteStats;
  wallClockMs: number;
  replicateMs: number[];
  compositeScore: number;
  skipped?: boolean;
 ...
 */
export function computeStats(results: NetworkWanWorkloadResult[], suite: typeof SUITE_CONFIG): NetworkWanSuiteStats {
 ...
 */
export function parseWorkloadResult(stdout: string): NetworkWanWorkloadResult {
  const lines = stdout.split(/\r?\n/);
  for (let i = lines.length - 1; i >= 0; i--) {
    const line = lines[i].trim();
    if (!line || !line.startsWith('{')) continue;
    try {
      const parsed = JSON.parse(line) as NetworkWanWorkloadResult;
      if (!parsed || typeof parsed !== 'object') continue;
      if (typeof parsed.suite !== 'string' || parsed.suite !== 'network-wan') continue;
      return parsed;
    } catch {
      continue;
    }
...
/**
 * Run the network-wan benchmark for one provider at the given replicate count.
 *
 * Each replicate:
 ...
 */
export async function runNetworkWanBenchmark(
  config: ProviderConfig & { replicas?: number },
): Promise<NetworkWanBenchmarkResult> {
  ...
  const iterations: NetworkWanWorkloadResult[] = [];
...
export async function writeNetworkWanResultsJson(results: NetworkWanBenchmarkResult[], outPath: string): Promise<void> {
  ...
  const output = {
    ...
    config: { mode: 'network-wan', ceiling: SUITE_CONFIG.ceiling, unit: SUITE_CONFIG.unit, timeoutMs: SUITE_CONFIG.timeoutMs },
...
function emptyStats(): NetworkWanSuiteStats {
  return { median: 0, p95: 0, p99: 0, min: 0, max: 0, successRate: 0, n: 0, scoreBeforeReliability: 0, compositeScore: 0, meta: {} };
}

type FailureReason = Extract<NetworkWanWorkloadResult, { ok: false }>['reason'];
...
function buildPayload(): Payload | PayloadErr {
  if (!fs.existsSync(BENCH_SCRIPT_PATH)) {
    return { ok: false, error: 'Workload script missing on network-wan: ' + BENCH_SCRIPT_PATH };
  }
...
function lastMeta(results: NetworkWanWorkloadResult[]): NetworkWanWorkloadMeta | undefined {
