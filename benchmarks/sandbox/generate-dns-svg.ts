/**
 * Render dns SVG bar chart from results/dns/latest.json.
...
import { SUITE_CONFIG, type DnsBenchmarkResult } from './dns.js';
...
function readLatest(): DnsBenchmarkResult[] | null {
  const dir = path.join(RESULTS_DIR, 'dns');
...
if (process.argv[1] && process.argv[1].endsWith('generate-dns-svg.ts')) {
  main();
}
