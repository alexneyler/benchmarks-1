...
const SUITE_CONFIG = {
  ...
  ceiling: 100,
  defaultReplicas: 3,
  workloadPath: 'download-workload.js',
  timeoutMs: 30_000,
};

const BENCH_SCRIPT_PATH = path.resolve(import.meta.dirname, '../scripts/download-workload.js');
const BENCH_STDOUT_PATH = path.resolve(import.meta.dirname, '../scripts/download-stdout.js');

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

export interface DownloadWorkloadMeta {
  cpuCount?: number;
  memoryMb?: number;
  workloadMs?: number;
...
export type DownloadWorkloadResult =
  | {
      ok: true;
      suite: 'download';
      metric: { value: number; unit: 'mb_per_s'; higherIsBetter: boolean };
      meta: DownloadWorkloadMeta;
      notes?: string;
    }
  | {
      ok: false;
...
export interface DownloadSuiteStats {
  median: number;
  p95: number;
  p99: number;
  min: number;
...
export interface DownloadBenchmarkResult {
  ...
  suite: 'download';
  mode: 'download';
  iterations: DownloadWorkloadResult[];
  summary: DownloadSuiteStats;
  wallClockMs: number;
  replicateMs: number[];
  compositeScore: number;
  skipped?: boolean;
 ...
 */
export function computeStats(results: DownloadWorkloadResult[], suite: typeof SUITE_CONFIG): DownloadSuiteStats {
 ...
 */
export function parseWorkloadResult(stdout: string): DownloadWorkloadResult {
  const lines = stdout.split(/\r?\n/);
  for (let i = lines.length - 1; i >= 0; i--) {
    const line = lines[i].trim();
    if (!line || !line.startsWith('{')) continue;
    try {
      const parsed = JSON.parse(line) as DownloadWorkloadResult;
      if (!parsed || typeof parsed !== 'object') continue;
      if (typeof parsed.suite !== 'string' || parsed.suite !== 'download') continue;
      return parsed;
    } catch {
      continue;
    }
...
/**
 * Run the download benchmark for one provider at the given replicate count.
 *
 * Each replicate:
...
export async function runDownloadBenchmark(
  ...
  const iterations: DownloadWorkloadResult[] = [];
  ...
  console.log('  [' + name + '/' + suite.id + '] ' + summary.n + '/' + replicas + ' OK · median ' + summary.median.toFixed(1) + ' MB/s · score ' + summary.compositeScore);
...
export async function writeDownloadResultsJson(results: DownloadBenchmarkResult[], outPath: string): Promise<void> {
  ...
  const output = {
    ...
    config: { mode: 'download', ceiling: SUITE_CONFIG.ceiling, unit: SUITE_CONFIG.unit, timeoutMs: SUITE_CONFIG.timeoutMs },
...
function buildPayload(): Payload | PayloadErr {
  if (!fs.existsSync(BENCH_SCRIPT_PATH)) {
    return { ok: false, error: 'Workload script missing on download: ' + BENCH_SCRIPT_PATH };
  }
...
function lastMeta(results: DownloadWorkloadResult[]): DownloadWorkloadMeta | undefined {
