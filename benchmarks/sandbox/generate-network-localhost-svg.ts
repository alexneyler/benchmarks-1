/**
 * Render network-localhost SVG bar chart from results/network_localhost/latest.json.
...
import { SUITE_CONFIG, type NetworkLocalhostBenchmarkResult } from './network-localhost.js';
...
function readLatest(): NetworkLocalhostBenchmarkResult[] | null {
  const dir = path.join(RESULTS_DIR, 'network_localhost');
...
function generateSvg(): string {
  ...
  parts.push('<text x="20" y="48" font-size="12" fill="#475569">unit: ' + unit + '  ceiling: ' + suite.ceiling + '  method: median, 2-sigma trim</text>');
  ...
  for (let i = 0; i < visible.length; i++) {
    ...
    const sublabel = 'n=' + r.summary.n + ' ' + suite.ceiling + ' ' + unit + ' ceiling';
...
function main(): void {
  ...
  const name = 'network_localhost.svg';
  const target = path.join(OUT_DIR, name);
  fs.writeFileSync(target, svg);
  console.log('Wrote ' + name + ' (' + svg.length + ' bytes)');
}

if (process.argv[1] && process.argv[1].endsWith('generate-network-localhost-svg.ts')) {
  main();
}
