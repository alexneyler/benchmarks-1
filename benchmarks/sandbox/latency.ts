...
const SUITE_CONFIG = {
  id: 'latency' as const,
  label: 'Network edge RTT',
  unit: 'rtt_ms' as const,
  higherIsBetter: false,
  ceiling: 35,
  defaultReplicas: 3,
  workloadPath: 'latency-workload.js',
  timeoutMs: 20_000,
};

const BENCH_SCRIPT_PATH = path.resolve(import.meta.dirname, '../scripts/latency-workload.js');
const BENCH_STDOUT_PATH = path.resolve(import.meta.dirname, '../scripts/latency-stdout.js');

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

export interface LatencyWorkloadMeta {
  cpuCount?: number;
  memoryMb?: number;
  workloadMs?: number;
...
export type LatencyWorkloadResult =
  | {
      ok: true;
      suite: 'latency';
      metric: { value: number; unit: 'rtt_ms'; higherIsBetter: boolean };
      meta: LatencyWorkloadMeta;
      notes?: string;
    }
  | {
      ok: false;
...
export interface LatencySuiteStats {
  median: number;
  p95: number;
  p99: number;
  min: number;
...
export interface LatencyBenchmarkResult {
  ...
  suite: 'latency';
  mode: 'latency';
  iterations: LatencyWorkloadResult[];
  summary: LatencySuiteStats;
  wallClockMs: number;
  replicateMs: number[];
  compositeScore: number;
  skipped?: boolean;
 ...
 */
export function computeStats(results: LatencyWorkloadResult[], suite: typeof SUITE_CONFIG): LatencySuiteStats {
 ...
 */
export function parseWorkloadResult(stdout: string): LatencyWorkloadResult {
  const lines = stdout.split(/\r?\n/);
  for (let i = lines.length - 1; i >= 0; i--) {
    const line = lines[i].trim();
    if (!line || !line.startsWith('{')) continue;
    try {
      const parsed = JSON.parse(line) as LatencyWorkloadResult;
      if (!parsed || typeof parsed !== 'object') continue;
      if (typeof parsed.suite !== 'string' || parsed.suite !== 'latency') continue;
      return parsed;
    } catch {
      continue;
    }
...
/**
 * Run the latency benchmark for one provider at the given replicate count.
 *
 * Each replicate:
...
export async function runLatencyBenchmark(
  ...
  const iterations: LatencyWorkloadResult[] = [];
...
export async function writeLatencyResultsJson(results: LatencyBenchmarkResult[], outPath: string): Promise<void> {
  ...
  const output = {
    ...
    config: { mode: 'latency', ceiling: SUITE_CONFIG.ceiling, unit: SUITE_CONFIG.unit, timeoutMs: SUITE_CONFIG.timeoutMs },
...
function buildPayload(): Payload | PayloadErr {
  if (!fs.existsSync(BENCH_SCRIPT_PATH)) {
    return { ok: false, error: 'Workload script missing on latency: ' + BENCH_SCRIPT_PATH };
  }
...
function lastMeta(results: LatencyWorkloadResult[]): LatencyWorkloadMeta | undefined {
