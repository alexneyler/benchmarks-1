/**
 * Render latency SVG bar chart from results/latency/latest.json.
...
import { SUITE_CONFIG, type LatencyBenchmarkResult } from './latency.js';
...
function readLatest(): LatencyBenchmarkResult[] | null {
  const dir = path.join(RESULTS_DIR, 'latency');
...
function main(): void {
  ...
  const name = 'latency.svg';
  const target = path.join(OUT_DIR, name);
  fs.writeFileSync(target, svg);
  console.log('Wrote ' + name + ' (' + svg.length + ' bytes)');
}

if (process.argv[1] && process.argv[1].endsWith('generate-latency-svg.ts')) {
  main();
}
