/**
 * Render disk SVG bar chart from results/disk/latest.json.
...
import { SUITE_CONFIG, type DiskBenchmarkResult } from './disk.js';
...
function readLatest(): DiskBenchmarkResult[] | null {
  const dir = path.join(RESULTS_DIR, 'disk');
...
if (process.argv[1] && process.argv[1].endsWith('generate-disk-svg.ts')) {
  main();
}
