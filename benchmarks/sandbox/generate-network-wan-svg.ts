/**
 * Render network-wan SVG bar chart from results/network_wan/latest.json.
...
import { SUITE_CONFIG, type NetworkWanBenchmarkResult } from './network-wan.js';
...
function readLatest(): NetworkWanBenchmarkResult[] | null {
  const dir = path.join(RESULTS_DIR, 'network_wan');
...
function main(): void {
  ...
  const name = 'network_wan.svg';
  const target = path.join(OUT_DIR, name);
  fs.writeFileSync(target, svg);
  console.log('Wrote ' + name + ' (' + svg.length + ' bytes)');
}

if (process.argv[1] && process.argv[1].endsWith('generate-network-wan-svg.ts')) {
  main();
}
