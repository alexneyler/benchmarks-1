/**
 * Render download SVG bar chart from results/download/latest.json.
...
import { SUITE_CONFIG, type DownloadBenchmarkResult } from './download.js';
...
function readLatest(): DownloadBenchmarkResult[] | null {
  const dir = path.join(RESULTS_DIR, 'download');
...
if (process.argv[1] && process.argv[1].endsWith('generate-download-svg.ts')) {
  main();
}
