/**
 * Render memory SVG bar chart from results/memory/latest.json.
...
import { SUITE_CONFIG, type MemoryBenchmarkResult } from './memory.js';
...
function readLatest(): MemoryBenchmarkResult[] | null {
  const dir = path.join(RESULTS_DIR, 'memory');
...
function main(): void {
  ...
  const name = 'memory.svg';
  const target = path.join(OUT_DIR, name);
  fs.writeFileSync(target, svg);
  console.log('Wrote ' + name + ' (' + svg.length + ' bytes)');
}

if (process.argv[1] && process.argv[1].endsWith('generate-memory-svg.ts')) {
  main();
}
