...
const SUITE_CONFIG = {
  ...
  ceiling: 2500,
  defaultReplicas: 3,
  workloadPath: 'disk-workload.js',
  timeoutMs: 90_000,
};

const BENCH_SCRIPT_PATH = path.resolve(import.meta.dirname, '../scripts/disk-workload.js');
const BENCH_STDOUT_PATH = path.resolve(import.meta.dirname, '../scripts/disk-stdout.js');

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

export interface DiskWorkloadMeta {
  cpuCount?: number;
  memoryMb?: number;
  workloadMs?: number;
...
export interface DiskSuiteStats {
  median: number;
  p95: number;
  p99: number;
  min: number;
...
export interface DiskBenchmarkResult {
  ...
  suite: 'disk';
  mode: 'disk';
  iterations: DiskWorkloadResult[];
  summary: DiskSuiteStats;
  wallClockMs: number;
  replicateMs: number[];
  compositeScore: number;
  skipped?: boolean;
 ...
 */
export function computeStats(results: DiskWorkloadResult[], suite: typeof SUITE_CONFIG): DiskSuiteStats {
 ...
 */
export function parseWorkloadResult(stdout: string): DiskWorkloadResult {
  const lines = stdout.split(/\r?\n/);
  for (let i = lines.length - 1; i >= 0; i--) {
    const line = lines[i].trim();
    if (!line || !line.startsWith('{')) continue;
    try {
      const parsed = JSON.parse(line) as DiskWorkloadResult;
      if (!parsed || typeof parsed !== 'object') continue;
      if (typeof parsed.suite !== 'string' || parsed.suite !== 'disk') continue;
      return parsed;
    } catch {
      continue;
    }
...
/**
 * Run the disk benchmark for one provider at the given replicate count.
 *
 * Each replicate:
...
export async function runDiskBenchmark(
  ...
  const iterations: DiskWorkloadResult[] = [];
...
export async function writeDiskResultsJson(results: DiskBenchmarkResult[], outPath: string): Promise<void> {
  ...
  const output = {
    ...
    config: { mode: 'disk', ceiling: SUITE_CONFIG.ceiling, unit: SUITE_CONFIG.unit, timeoutMs: SUITE_CONFIG.timeoutMs },
...
function lastMeta(results: DiskWorkloadResult[]): DiskWorkloadMeta | undefined {
