...
const SUITE_CONFIG = {
  ...
  ceiling: 30000,
  defaultReplicas: 3,
  workloadPath: 'memory-workload.js',
  timeoutMs: 60_000,
};

const BENCH_SCRIPT_PATH = path.resolve(import.meta.dirname, '../scripts/memory-workload.js');
const BENCH_STDOUT_PATH = path.resolve(import.meta.dirname, '../scripts/memory-stdout.js');

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

export interface MemoryWorkloadMeta {
  cpuCount?: number;
  memoryMb?: number;
  workloadMs?: number;
...
export interface MemorySuiteStats {
  median: number;
  p95: number;
  p99: number;
  min: number;
...
export interface MemoryBenchmarkResult {
  ...
  suite: 'memory';
  mode: 'memory';
  iterations: MemoryWorkloadResult[];
  summary: MemorySuiteStats;
  wallClockMs: number;
  replicateMs: number[];
  compositeScore: number;
  skipped?: boolean;
 ...
 */
export function computeStats(results: MemoryWorkloadResult[], suite: typeof SUITE_CONFIG): MemorySuiteStats {
 ...
 */
export function parseWorkloadResult(stdout: string): MemoryWorkloadResult {
  const lines = stdout.split(/\r?\n/);
  for (let i = lines.length - 1; i >= 0; i--) {
    const line = lines[i].trim();
    if (!line || !line.startsWith('{')) continue;
    try {
      const parsed = JSON.parse(line) as MemoryWorkloadResult;
      if (!parsed || typeof parsed !== 'object') continue;
      if (typeof parsed.suite !== 'string' || parsed.suite !== 'memory') continue;
      return parsed;
    } catch {
      continue;
    }
...
/**
 * Run the memory benchmark for one provider at the given replicate count.
 *
 * Each replicate:
...
export async function runMemoryBenchmark(
  ...
  const iterations: MemoryWorkloadResult[] = [];
...
export async function writeMemoryResultsJson(results: MemoryBenchmarkResult[], outPath: string): Promise<void> {
  ...
  const output = {
    ...
    config: { mode: 'memory', ceiling: SUITE_CONFIG.ceiling, unit: SUITE_CONFIG.unit, timeoutMs: SUITE_CONFIG.timeoutMs },
...
function lastMeta(results: MemoryWorkloadResult[]): MemoryWorkloadMeta | undefined {
