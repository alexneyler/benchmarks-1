...
import { computeStats, scoreMetric, SUITE_CONFIG } from '../download.js';
import type { DownloadWorkloadResult } from '../download.js';

describe('scoring: download', () => {
  ...
  test('computeStats handles all-failure iterations', () => {
    const results: DownloadWorkloadResult[] = [
      { ok: false, suite: 'download', reason: 'error', error: 'test', meta: {} },
      { ok: false, suite: 'download', reason: 'timeout', error: 'test', meta: {} },
    ];
  ...
  test('computeStats produces median and score for successful runs', () => {
    ...
    const results: DownloadWorkloadResult[] = [
      { ok: true, suite: 'download', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
      { ok: true, suite: 'download', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
      { ok: true, suite: 'download', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
    ];
