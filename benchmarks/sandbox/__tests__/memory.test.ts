...
import { computeStats, scoreMetric, SUITE_CONFIG } from '../memory.js';
import type { MemoryWorkloadResult } from '../memory.js';

describe('scoring: memory', () => {
  ...
  test('computeStats handles all-failure iterations', () => {
    const results: MemoryWorkloadResult[] = [
      { ok: false, suite: 'memory', reason: 'error', error: 'test', meta: {} },
      { ok: false, suite: 'memory', reason: 'timeout', error: 'test', meta: {} },
    ];
  ...
  test('computeStats produces median and score for successful runs', () => {
    ...
    const results: MemoryWorkloadResult[] = [
      { ok: true, suite: 'memory', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
      { ok: true, suite: 'memory', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
      { ok: true, suite: 'memory', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
    ];
