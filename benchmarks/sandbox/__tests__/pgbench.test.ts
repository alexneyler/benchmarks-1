...
import { computeStats, scoreMetric, SUITE_CONFIG } from '../pgbench.js';
import type { PgbenchWorkloadResult } from '../pgbench.js';

describe('scoring: pgbench', () => {
  ...
  test('computeStats handles all-failure iterations', () => {
    const results: PgbenchWorkloadResult[] = [
      { ok: false, suite: 'pgbench', reason: 'error', error: 'test', meta: {} },
      { ok: false, suite: 'pgbench', reason: 'timeout', error: 'test', meta: {} },
    ];
  ...
  test('computeStats produces median and score for successful runs', () => {
    ...
    const results: PgbenchWorkloadResult[] = [
      { ok: true, suite: 'pgbench', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
      { ok: true, suite: 'pgbench', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
      { ok: true, suite: 'pgbench', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
    ];
