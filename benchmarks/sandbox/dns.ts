...
const SUITE_CONFIG = {
  ...
  ceiling: 10,
  defaultReplicas: 3,
  workloadPath: 'dns-workload.js',
  timeoutMs: 20_000,
};

const BENCH_SCRIPT_PATH = path.resolve(import.meta.dirname, '../scripts/dns-workload.js');
const BENCH_STDOUT_PATH = path.resolve(import.meta.dirname, '../scripts/dns-stdout.js');

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

export interface DnsWorkloadMeta {
  cpuCount?: number;
  memoryMb?: number;
  workloadMs?: number;
...
export interface DnsSuiteStats {
  median: number;
  p95: number;
  p99: number;
  min: number;
...
export interface DnsBenchmarkResult {
  ...
  suite: 'dns';
  mode: 'dns';
  iterations: DnsWorkloadResult[];
  summary: DnsSuiteStats;
  wallClockMs: number;
  replicateMs: number[];
  compositeScore: number;
  skipped?: boolean;
 ...
 */
export function computeStats(results: DnsWorkloadResult[], suite: typeof SUITE_CONFIG): DnsSuiteStats {
 ...
 */
export function parseWorkloadResult(stdout: string): DnsWorkloadResult {
  const lines = stdout.split(/\r?\n/);
  for (let i = lines.length - 1; i >= 0; i--) {
    const line = lines[i].trim();
    if (!line || !line.startsWith('{')) continue;
    try {
      const parsed = JSON.parse(line) as DnsWorkloadResult;
      if (!parsed || typeof parsed !== 'object') continue;
      if (typeof parsed.suite !== 'string' || parsed.suite !== 'dns') continue;
      return parsed;
    } catch {
      continue;
    }
...
/**
 * Run the dns benchmark for one provider at the given replicate count.
 *
 * Each replicate:
...
export async function runDnsBenchmark(
  ...
  const iterations: DnsWorkloadResult[] = [];
...
export async function writeDnsResultsJson(results: DnsBenchmarkResult[], outPath: string): Promise<void> {
  ...
  const output = {
    ...
    config: { mode: 'dns', ceiling: SUITE_CONFIG.ceiling, unit: SUITE_CONFIG.unit, timeoutMs: SUITE_CONFIG.timeoutMs },
...
function buildPayload(): Payload | PayloadErr {
  if (!fs.existsSync(BENCH_SCRIPT_PATH)) {
    return { ok: false, error: 'Workload script missing on dns: ' + BENCH_SCRIPT_PATH };
  }
...
function lastMeta(results: DnsWorkloadResult[]): DnsWorkloadMeta | undefined {
