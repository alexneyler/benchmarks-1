...
const SUITE_CONFIG = {
  id: 'network-localhost' as const,
  label: 'Network localhost throughput',
  unit: 'gb_per_s' as const,
  higherIsBetter: true,
  ceiling: 3,
  defaultReplicas: 3,
  workloadPath: 'network-localhost-workload.js',
  timeoutMs: 30_000,
};

const BENCH_SCRIPT_PATH = path.resolve(import.meta.dirname, '../scripts/network-localhost-workload.js');
const BENCH_STDOUT_PATH = path.resolve(import.meta.dirname, '../scripts/network-localhost-stdout.js');

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

export interface NetworkLocalhostWorkloadMeta {
  cpuCount?: number;
  memoryMb?: number;
  workloadMs?: number;
...
export type NetworkLocalhostWorkloadResult =
  | {
      ok: true;
      suite: 'network-localhost';
      metric: { value: number; unit: 'gb_per_s'; higherIsBetter: boolean };
      meta: NetworkLocalhostWorkloadMeta;
      notes?: string;
    }
  | {
      ok: false;
...
export interface NetworkLocalhostSuiteStats {
  median: number;
  p95: number;
  p99: number;
  min: number;
...
export interface NetworkLocalhostBenchmarkResult {
  provider: string;
  suite: 'network-localhost';
  mode: 'network-localhost';
  iterations: NetworkLocalhostWorkloadResult[];
  summary: NetworkLocalhostSuiteStats;
  wallClockMs: number;
  replicateMs: number[];
  compositeScore: number;
  skipped?: boolean;
 ...
 */
export function computeStats(results: NetworkLocalhostWorkloadResult[], suite: typeof SUITE_CONFIG): NetworkLocalhostSuiteStats {
 ...
 */
export function parseWorkloadResult(stdout: string): NetworkLocalhostWorkloadResult {
  const lines = stdout.split(/\r?\n/);
  for (let i = lines.length - 1; i >= 0; i--) {
    const line = lines[i].trim();
    if (!line || !line.startsWith('{')) continue;
    try {
      const parsed = JSON.parse(line) as NetworkLocalhostWorkloadResult;
      if (!parsed || typeof parsed !== 'object') continue;
      if (typeof parsed.suite !== 'string' || parsed.suite !== 'network-localhost') continue;
      return parsed;
    } catch {
      continue;
    }
...
/**
 * Run the network-localhost benchmark for one provider at the given replicate count.
 *
 * Each replicate:
 ...
 */
export async function runNetworkLocalhostBenchmark(
  config: ProviderConfig & { replicas?: number },
): Promise<NetworkLocalhostBenchmarkResult> {
  ...
  const iterations: NetworkLocalhostWorkloadResult[] = [];
  ...
  console.log('  [' + name + '/' + suite.id + '] ' + summary.n + '/' + replicas + ' OK · median ' + summary.median.toFixed(1) + ' GB/s · score ' + summary.compositeScore);
...
export async function writeNetworkLocalhostResultsJson(results: NetworkLocalhostBenchmarkResult[], outPath: string): Promise<void> {
  ...
  const output = {
    ...
    config: { mode: 'network-localhost', ceiling: SUITE_CONFIG.ceiling, unit: SUITE_CONFIG.unit, timeoutMs: SUITE_CONFIG.timeoutMs },
...
function emptyStats(): NetworkLocalhostSuiteStats {
  return { median: 0, p95: 0, p99: 0, min: 0, max: 0, successRate: 0, n: 0, scoreBeforeReliability: 0, compositeScore: 0, meta: {} };
}

type FailureReason = Extract<NetworkLocalhostWorkloadResult, { ok: false }>['reason'];
...
function buildPayload(): Payload | PayloadErr {
  if (!fs.existsSync(BENCH_SCRIPT_PATH)) {
    return { ok: false, error: 'Workload script missing on network-localhost: ' + BENCH_SCRIPT_PATH };
  }
...
function lastMeta(results: NetworkLocalhostWorkloadResult[]): NetworkLocalhostWorkloadMeta | undefined {
