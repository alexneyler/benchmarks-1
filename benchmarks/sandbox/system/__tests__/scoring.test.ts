...
describe('scoring: system', () => {
  ...
  test('computeStats handles all-failure iterations', () => {
    const results: WorkloadResult[] = [
      { ok: false, suite: 'system', reason: 'error', error: 'test', meta: {} },
      { ok: false, suite: 'system', reason: 'timeout', error: 'test', meta: {} },
    ];
  ...
  test('computeStats produces median and score for successful runs', () => {
    ...
    const results: WorkloadResult[] = [
      { ok: true, suite: 'system', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
      { ok: true, suite: 'system', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
      { ok: true, suite: 'system', metric: { value: val, unit: suite.unit, higherIsBetter: suite.higherIsBetter }, meta: {} },
    ];
