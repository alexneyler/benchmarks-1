...
/**
 * In-sandbox workload for `hpc-disk`.
 *
 ...
 *
 * Reported metric: average MB/s across the three phases. We deliberately
 ...
 *
 * Calibration target (ceiling): 200 MB/s. S3-backed networked disks drop
 * below this; tmpfs-backed providers trivially clear 500+ MB/s.
 */
...
async function run() {
  const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'hpc-disk-'));
  ...
  // Best-effort cleanup; ok if this races with sandbox destory.
