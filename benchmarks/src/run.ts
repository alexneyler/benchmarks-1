...
/**
 * Per-benchmark runner for download. Mirrors the cpu-node standalone suite:
 * direct provider iteration, per-day results, and a generated SVG chart.
 */
async function runDownloadBenchmarkSuite(toRun: typeof providers): Promise<void> {
  const suite = DOWNLOAD_CONFIG;
  const replicas = explicitReplicas ?? suite.defaultReplicas;
  ...
  const results = [];
  for (const providerConfig of toRun) {
    const result = await runDownloadBenchmark({ ...providerConfig, replicas });
    results.push(result);
  }
  ...
  for (const r of results) {
    ...
    console.log(`${r.provider}: median ${r.summary.median.toFixed(1)} MB/s · score ${r.compositeScore} (${r.summary.n}/${r.iterations.length} OK)`);
  }
  ...
  fs.mkdirSync(resultsDir, { recursive: true });
  const outPath = path.join(resultsDir, `${timestamp}.json`);
  await writeDownloadResultsJson(results, outPath);
  const latestPath = path.join(resultsDir, 'latest.json');
  fs.copyFileSync(outPath, latestPath);
  console.log(`Copied latest: ${latestPath}`);
  ...
  try {
    const { spawnSync } = await import('node:child_process');
    const scriptPath = path.resolve(__dirname, '../sandbox/generate-download-svg.ts');
    spawnSync('npx', ['tsx', scriptPath], { stdio: 'inherit' });
  } catch (err) {
    console.warn(`    [generate-svg] skipped: ${err instanceof Error ? err.message : String(err)}`);
  }
...
/**
 * Per-benchmark runner for DNS. Mirrors the other standalone suites:
 * direct provider iteration, per-day results, and a generated SVG chart.
 */
async function runDnsBenchmarkSuite(toRun: typeof providers): Promise<void> {
  const suite = DNS_CONFIG;
  const replicas = explicitReplicas ?? suite.defaultReplicas;
  ...
  const results = [];
  for (const providerConfig of toRun) {
    const result = await runDnsBenchmark({ ...providerConfig, replicas });
    results.push(result);
  }
  ...
  fs.mkdirSync(resultsDir, { recursive: true });
  const outPath = path.join(resultsDir, `${timestamp}.json`);
  await writeDnsResultsJson(results, outPath);
  const latestPath = path.join(resultsDir, 'latest.json');
  fs.copyFileSync(outPath, latestPath);
  console.log(`Copied latest: ${latestPath}`);
  ...
  try {
    const { spawnSync } = await import('node:child_process');
    const scriptPath = path.resolve(__dirname, '../sandbox/generate-dns-svg.ts');
    spawnSync('npx', ['tsx', scriptPath], { stdio: 'inherit' });
  } catch (err) {
    console.warn(`    [generate-svg] skipped: ${err instanceof Error ? err.message : String(err)}`);
  }
...
/**
 * Per-benchmark runner for network-WAN. Mirrors the other standalone suites:
 * direct provider iteration, per-day results, and an SVG chart.
 */
async function runNetworkWanBenchmarkSuite(toRun: typeof providers): Promise<void> {
  const suite = NETWORK_WAN_CONFIG;
  const replicas = explicitReplicas ?? suite.defaultReplicas;
  ...
  const results = [];
  for (const providerConfig of toRun) {
    const result = await runNetworkWanBenchmark({ ...providerConfig, replicas });
    results.push(result);
  }

  console.log('\n--- Network-WAN Benchmark Results ---');
  for (const r of results) {
    if (r.skipped) {
      console.log(`${r.provider}: SKIPPED (${r.skipReason})`);
      continue;
  ...
  fs.mkdirSync(resultsDir, { recursive: true });
  const outPath = path.join(resultsDir, `${timestamp}.json`);
  await writeNetworkWanResultsJson(results, outPath);
  const latestPath = path.join(resultsDir, 'latest.json');
  fs.copyFileSync(outPath, latestPath);
  console.log(`Copied latest: ${latestPath}`);
  ...
  try {
    const { spawnSync } = await import('node:child_process');
    const scriptPath = path.resolve(__dirname, '../sandbox/generate-network-wan-svg.ts');
    spawnSync('npx', ['tsx', scriptPath], { stdio: 'inherit' });
  } catch (err) {
    console.warn(`    [generate-svg] skipped: ${err instanceof Error ? err.message : String(err)}`);
  }
