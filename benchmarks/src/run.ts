...
async function runSandboxDax(toRun: typeof providers, daxIterations: number = iterations): Promise<void> {
  console.log('\n' + '='.repeat(70));
...
async function main() {
  ...
  if (modes[0] === 'ai-gateway') {
    ...
    const toRun = selectProviders(aiGatewayProviders, providerNames);
  ...
  // Handle sandbox-dax mode separately (disk / CPU / pause-resume sub-test,
  // same provider registry as the regular sandbox benchmark)
...
async function runFromConfigFile(configPath: string): Promise<void> {
  ...
  if (mode === 'sandbox-dax') {
    await runSandboxDax(toRun, settings.iterations);
  } else {
    await runMode(mode as BenchmarkMode, toRun, settings);
  }
...
function printDryRunSummary(
  ...
  if (mode === 'sequential' || mode === 'sandbox-dax') {
