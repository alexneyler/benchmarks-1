 ...
 *
 * Usage: tsx src/ingest.ts --type sandbox|storage|browser [--dir results]
...
if (!typeArg || !['sandbox', 'storage', 'browser'].includes(typeArg)) {
  console.error('Usage: tsx src/ingest.ts --type sandbox|storage|browser [--dir results]');
  process.exit(1);
}

const dirFlagIndex = args.indexOf('--dir');
const RESULTS_DIR = (dirFlagIndex >= 0 ? args[dirFlagIndex + 1] : undefined) || 'results';
...
async function ingestSandbox() {
  for (const { dir, mode } of SANDBOX_MODES) {
    const latestPath = path.join(ROOT, RESULTS_DIR, dir, 'latest.json');
...
async function ingestStorage() {
  const storageDir = path.join(ROOT, RESULTS_DIR, 'storage');
  if (!fs.existsSync(storageDir)) {
    console.log(`Skipping storage: ${RESULTS_DIR}/storage/ not found`);
    return;
  }
...
async function ingestBrowser() {
  const latestPath = path.join(ROOT, RESULTS_DIR, 'browser', 'latest.json');
  if (!fs.existsSync(latestPath)) {
